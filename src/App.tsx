    ...
    children: [
      {
        path: 'test',
        element: (
          <div onClick={() => alert(location.href)}>
            (click to view href)This is /detail/test (subpage)
          </div>
        ),
      },
    ],
...
export const versionContext = createContext<VersionParams>({
  frontend: '?',
  capa: '?',
})

export default function App() {
  const [versionParams, setVersionParams] = useState<VersionParams>({
    frontend: version,
    capa: 'loading',
  })
  useEffect(() => {
    setLatestRenderedVersion(version).then(() => {
      getVersion().then(
        (v) => setVersionParams((p) => ({ ...p, capa: v })),
        console.error,
      )
      updateFrontend().then((v) => {
        if (v) {
          setVersionParams((p) => ({ ...p, newVersion: v }))
        }
      })
    })
  }, [])
