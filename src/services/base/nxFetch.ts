import { CapacitorCookies, CapacitorHttp, HttpOptions } from '@capacitor/core'
import '@/utils/extendHeaders'
import { env } from '@/utils/env.ts'

let cookieJar: import('tough-cookie').CookieJar | null = null

export async function initNodeCookieJar() {
...
async function nxFetchBase(
  ...
  console.info('nxFetch', req, resp)

  const { url: respUrl, data: respData, status, headers: respHeaders } = resp
 ...
 */
const nxFetch: typeof nxFetchBase & typeof nxFetchExtend = Object.assign(
  nxFetchBase,
  nxFetchExtend,
)
