...
function Head(){
    ...
    interface Tips {                     //天气与温馨提示的对象数组
        weather : string,
        tip : string
    }
    let tips: Tips[] = [  {weather : '下雨' , tip : '今日有雨，记得带伞哦！'},
    ...
    interface calendarModel {                       //calendar部分接口与数据
        year: number,
        month: number,
        day: number,
        season: string,
    ...
    interface weatherModel {                       //weather部分接口与数据
        weather: string,
        temperature: {
            min: number,
            max: number
    ...
    const tip = (tips.find(item => item.weather === weatherCondition.weather) as Tips).tip        //找到对应天气的温馨提醒
